...
df_1 = pd.DataFrame(columns=["Depressed", "Not Depressed"])
for i in range(10):
    prediction = example(df.iloc[i,0])
    print(df.iloc[i,0],"----",prediction)
    if prediction == "depressed":
         df_1 = df_1.append({"Depressed":df.iloc[i,0]}, ignore_index = True)
#    else:
#         df_1 = df_1.append({"Not Depressed":df.iloc[i,0]}, ignore_index = True)
